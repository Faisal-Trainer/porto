<x-app-layout>
    ...
    @section('og_image', $post->image ? asset('storage/' . $post->image) : ($post->is_journal ?
        asset('banners/journal.webp') : asset('banners/article.webp')))
    ...
    @section('twitter_image', $post->image ? asset('storage/' . $post->image) : ($post->is_journal ?
        asset('banners/journal.webp') : asset('banners/article.webp')))
        ...
        @push('schemas')
            @php
                $ogImage = $post->image
                    ...
                    : ($post->is_journal
                        ? asset('banners/journal.webp')
                        : asset('banners/article.webp'));
        ...
        <article class="min-h-screen pt-28 pb-12 md:pt-36 md:pb-24 px-4 md:px-8 bg-white">
            <div class="max-w-5xl mx-auto flex flex-col lg:flex-row gap-8 xl:gap-12 relative">
                ...
                <div class="flex-1 min-w-0 max-w-3xl">
                    ...
                    <header class="mb-12">
                        <div class="flex items-center gap-3 mb-6">
                            ...
                            <span
                                class="text-gray-500 text-[10px] font-bold uppercase tracking-widest flex items-center gap-1">
                    ...
                    <div
                        class="prose prose-purple max-w-none prose-lg md:prose-xl
                        prose-headings:text-(--color-primary-950) prose-headings:font-black prose-headings:tracking-tight
                        prose-p:text-gray-700 prose-p:leading-relaxed
                        prose-a:text-(--color-primary-600) prose-a:font-bold prose-a:no-underline prose-a:border-b-2 prose-a:border-primary-100 hover:prose-a:border-(--color-primary-600) prose-a:transition-all
                        prose-strong:text-(--color-primary-950)
                        prose-blockquote:border-l-4 prose-blockquote:border-(--color-primary-500) prose-blockquote:bg-(--color-primary-50)/50 prose-blockquote:rounded-r-2xl prose-blockquote:px-6 prose-blockquote:italic
                        prose-img:rounded-[2.5rem] prose-img:shadow-2xl prose-img:border prose-img:border-black/5
                        prose-code:text-(--color-primary-600) prose-code:bg-(--color-primary-50) prose-code:px-1.5 prose-code:py-0.5 prose-code:rounded-lg prose-code:before:content-none prose-code:after:content-none
                        prose-li:marker:text-(--color-primary-600)
                        selection:bg-(--color-primary-200) selection:text-(--color-primary-950)">
                    ...
                    <div class="hidden lg:flex flex-col gap-3 fixed left-8 top-1/2 -translate-y-1/2 z-30">
                        <a href="https://wa.me/?text={{ urlencode($post->title . ' ' . url()->current()) }}"
                            target="_blank"
                            class="w-12 h-12 flex items-center justify-center bg-green-500 text-white rounded-full shadow-lg hover:scale-110 hover:-rotate-12 transition-all duration-300"
                            title="Share ke WhatsApp">
                            <i class="fi fi-brands-whatsapp text-xl"></i>
                        </a>
                        <a href="https://www.linkedin.com/sharing/share-offsite/?url={{ url()->current() }}"
                            target="_blank"
                            class="w-12 h-12 flex items-center justify-center bg-[#0077b5] text-white rounded-full shadow-lg hover:scale-110 hover:rotate-12 transition-all duration-300"
                            title="Share ke LinkedIn">
                            <i class="fi fi-brands-linkedin text-xl"></i>
                        </a>
                        <button
                            @click="navigator.clipboard.writeText('{{ url()->current() }}'); $dispatch('notify', { message: 'Link berhasil disalin!' })"
                            class="w-12 h-12 flex items-center justify-center bg-white dark:bg-white/10 text-(--color-primary-600) dark:text-(--color-primary-400) rounded-full shadow-lg hover:scale-110 transition-all duration-300 border border-gray-100 dark:border-white/10"
                            title="Salin Link">
                    ...
                    <div
                        ...
                        <div class="flex flex-wrap gap-2">
                            @if ($post->meta_keywords)
                                @foreach (explode(',', $post->meta_keywords) as $tag)
                                    <span
                                        class="px-3 py-1 bg-gray-100 text-gray-600 text-[10px] font-bold rounded-full uppercase tracking-wider">#{{ trim($tag) }}</span>
                                @endforeach
                            @endif
                        </div>
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <a href="https://api.whatsapp.com/send?text={{ urlencode($post->title . ' - ' . route('blog.show', $post->slug)) }}"
                                target="_blank"
                                class="w-10 h-10 rounded-full bg-green-500 text-white flex items-center justify-center hover:scale-110 transition-transform shadow-md">
                                <i class="fi fi-brands-whatsapp text-lg mt-1"></i>
                            </a>
                            <a href="https://twitter.com/intent/tweet?url={{ urlencode(route('blog.show', $post->slug)) }}&text={{ urlencode($post->title) }}"
                                target="_blank"
                                class="w-10 h-10 rounded-full bg-black text-white flex items-center justify-center hover:scale-110 transition-transform shadow-md">
                            ...
                            <a href="https://www.linkedin.com/sharing/share-offsite/?url={{ urlencode(route('blog.show', $post->slug)) }}"
                                target="_blank"
                                class="w-10 h-10 rounded-full bg-blue-600 text-white flex items-center justify-center hover:scale-110 transition-transform shadow-md">
                        ...
                        <section class="mt-20 pt-16 border-t border-gray-100">
                            <div class="flex items-center justify-between mb-10">
                                <div>
                                    ...
                                    <p class="text-sm text-black mt-1">Mungkin Anda juga tertarik membaca ini.</p>
                                </div>
                            </div>
                ...
                <aside class="hidden lg:block w-64 shrink-0">
                    <div
                        class="sticky top-32 bg-(--color-primary-50)/50 backdrop-blur-sm p-5 rounded-2xl border border-(--color-primary-100)">
                        <h3 class="text-xs font-bold uppercase tracking-widest text-(--color-primary-500) mb-4">Daftar Isi
                        </h3>
        ...
        @push('modals')
            ...
            <div id="reading-progress-container"
                class="fixed top-0 left-0 w-full h-1.5 bg-transparent z-100 pointer-events-none">
                <div id="reading-progress-bar"
                    class="h-full bg-linear-to-r from-(--color-primary-400) to-(--color-primary-600) w-0 rounded-r-full shadow-[0_0_8px_rgba(147,51,234,0.8)] transition-all duration-150 ease-out">
                </div>
            </div>

            {{-- Image Lightbox Modal --}}
            <div x-data="{ open: false, src: '' }" @open-lightbox.window="src = $event.detail.src; open = true" x-show="open" x-cloak
                ...
                <div class="relative max-w-5xl w-full h-full flex items-center justify-center">
                    <img :src="src" class="max-w-full max-h-full object-contain rounded-2xl shadow-2xl"
                        @click.stop>
                    <button @click="open = false"
                        class="absolute top-4 right-4 w-12 h-12 flex items-center justify-center bg-white/10 hover:bg-white/20 text-white rounded-full transition-all">
                    ...
                    if (contentDiv && tocContainer) {
                        ...
                        if (headings.length > 0) {
                            headings.forEach(heading => {
                                ...
                                li.innerHTML =
                                    `<a href="#${heading.id}" class="block text-gray-500 dark:text-gray-400 hover:text-(--color-primary-600) dark:hover:text-(--color-primary-400) hover:translate-x-1 transition-all duration-300 line-clamp-2">${heading.textContent}</a>`;
                        ...
                        codeBlocks.forEach(block => {
                            ...
                            copyBtn.className =
                                'copy-btn absolute top-3 right-3 p-2 bg-white/10 hover:bg-white/20 text-white rounded-lg opacity-0 group-hover:opacity-100 transition-all duration-300 flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest backdrop-blur-sm border border-white/10';
